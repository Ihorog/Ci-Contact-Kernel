...
async function runWithR2Authority(loop, payload, options = {}) {
  ...
  const sha = intake.correlation_id.slice('corr-'.length);
  return loop.run(payload, {
    ...options,
    authority,
    observed_execution: {
      sha,
      verified: true,
      evidence_refs: [{ ref: 'test/check', sha }]
    }
  });
}
...
test('MaintainerLoop: Negative Test 1 & 2 - Duplicate event and retry are idempotent', async () => {
  ...
  const res1 = await runWithR2Authority(loop, payload);
...
test('MaintainerLoop: Negative Test 4 - Policy/schema change invalidates fast path', async () => {
  ...
  const run1 = await runWithR2Authority(loop, payload1);
  ...
  const run2 = await runWithR2Authority(loop, payload2);
...
test('MaintainerLoop: Negative Test 6 - Failing dependency bulkhead does not block separate bulkhead', async () => {
  ...
  const resA = await runWithR2Authority(loop, payloadA, { simulateTestFailure: true });
  assert.equal(resA.status, 'VERIFICATION_FAILED');

  const resB = await runWithR2Authority(loop, payloadB);
...
test('MaintainerLoop: Negative Test 7 - Unresolved review or merge conflict blocks merge', async () => {
  ...
  const res1 = await runWithR2Authority(loop, payload1, { hasUnresolvedReview: true });
  ...
  const res2 = await runWithR2Authority(loop, payload2, { hasMergeConflict: true });
...
test('MaintainerLoop: Negative Test 8 - AI proposal does not become code without mechanical verification', async () => {
  ...
  const res = await runWithR2Authority(loop, payload, { aiProposalUnverified: true });
...
test('MaintainerLoop: Negative Test 9 - Self-modified instruction with regression automatically rolls back', async () => {
  ...
  const res = await runWithR2Authority(loop, payload, {
...
test('MaintainerLoop: Negative Test 11 - Repeated maintainer run is idempotent', async () => {
  ...
  const run1 = await runWithR2Authority(loop, payload);
  assert.equal(run1.status, 'COMPLETED');
