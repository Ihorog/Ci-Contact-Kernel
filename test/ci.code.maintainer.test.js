...
function approvedRunOptions(loop, payload, options = {}) {
  const intake = loop.eventIntake.intake(payload);
  const sha = crypto.randomUUID().replaceAll('-', '');
  const riskClass = loop.repoRegistry.get(payload.repository.full_name)?.risk_class || 'R2';
  return {
    ...options,
    authority: loop.approve(intake.correlation_id, {
    ...
    observed_execution: {
      sha,
      verified: true,
      evidence_refs: [{ ref: 'test/verified', sha }]
    }
  };
}
...
test('Contracts: CiCoordinate, CiCapability, CiContact, CiCompletion', () => {
  ...
  const verif = completion.verifyTask(task, { status: 'SUCCESS' }, {
    ...
    observed: true,
    verified: true,
    evidence_refs: [{ ref: 'test/verified', sha: 'abc1234' }]
  });
...
test('MaintainerLoop: Negative Test 1 & 2 - Duplicate event and retry are idempotent', async () => {
  ...
  const res1 = await loop.run(payload, approvedRunOptions(loop, payload));
...
test('MaintainerLoop: Negative Test 4 - Policy/schema change invalidates fast path', async () => {
  ...
  const run1 = await loop.run(payload1, approvedRunOptions(loop, payload1));
  ...
  const run2 = await loop.run(payload2, approvedRunOptions(loop, payload2));
...
test('MaintainerLoop: Negative Test 6 - Failing dependency bulkhead does not block separate bulkhead', async () => {
  ...
  const resA = await loop.run(payloadA, approvedRunOptions(loop, payloadA, { simulateTestFailure: true }));
  assert.equal(resA.status, 'VERIFICATION_FAILED');

  const resB = await loop.run(payloadB, approvedRunOptions(loop, payloadB));
...
test('MaintainerLoop: Negative Test 7 - Unresolved review or merge conflict blocks merge', async () => {
  ...
  const res1 = await loop.run(payload1, approvedRunOptions(loop, payload1, { hasUnresolvedReview: true }));
  ...
  const res2 = await loop.run(payload2, approvedRunOptions(loop, payload2, { hasMergeConflict: true }));
...
test('MaintainerLoop: Negative Test 8 - AI proposal does not become code without mechanical verification', async () => {
  ...
  const res = await loop.run(payload, approvedRunOptions(loop, payload, { aiProposalUnverified: true }));
...
test('MaintainerLoop: Negative Test 9 - Self-modified instruction with regression automatically rolls back', async () => {
  ...
  const res = await loop.run(payload, approvedRunOptions(loop, payload, {
    isSelfModifiedInstruction: true,
...
test('MaintainerLoop: Negative Test 11 - Repeated maintainer run is idempotent', async () => {
  ...
  const run1 = await loop.run(payload, approvedRunOptions(loop, payload));
  assert.equal(run1.status, 'COMPLETED');
...
test('HTTP API: /ci/registry/repos, /ci/maintainer/intake, /ci/maintainer/run, /ci/digest', async () => {
  ...
  const server = app.createApp({ env: { CI_MAINTAINER_TOKEN: token } });
  ...
  assert.match(runRes.body.result.reason, /requires repository policy or user approval/);
